...
struct Lexer::TokenPattern {
    ...
    TokenType type;
    bool isKeyword;
    TokenPattern(const string& regexStr, TokenType t ,bool keyword=false) : pattern(regexStr,regex_constants::ECMAScript), type(t),isKeyword(keyword) {}
};
                ...
                if (!match.empty()&& match.position()==0)
                {
                    ...
                    if (pattern.isKeyword) {
                        ...
                        if (nextPos < input.length() && 
                            (isalnum(input[nextPos]) || input[nextPos] == '_')) {
                            continue; 
                        }
